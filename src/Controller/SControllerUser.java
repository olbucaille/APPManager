 ...
 * 
 * sert � gerer tous ce qui � rapport � l'objet User ( voir en BDD)
 */
...
public class SControllerUser extends HttpServlet {
	 ...
	 * ajoute un utilsisateur, � terme, l'enregistre dans la bdd si premiere connexion avec un statut de base modifi� par la suite 
	 * par l'admin
	 * est ensuite redirig� dsur la page qui lui convient ( eleve, tuteur (et bon groupe charg� si besoin),....)
	 */
			...
			if(arrayUser != null)
			{
				for(int i=0;i<arrayUser.length;i++)
				{
					...
					if(Role.equals("Module Manager")) 
					{
						...
						User.updatePriviledge(arrayUser[i],false,false,true,false);//si Module manager, alors on update que Module manager, sinon, c'est autres valeures sont �cras�es...
					}
					
						t.addOrReplaceTeamUser(arrayUser[i],Team);
						
						if(Role.equals("Tutor")) 
							User.updatePriviledge(arrayUser[i],false,true,false,false);//si Module manager, alors on update que Module manager, sinon, c'est autres valeures sont �cras�es...
						else if(Role.equals("Student")) 
							User.updatePriviledge(arrayUser[i],true,false,false,false);//si Module manager, alors on update que Module manager, sinon, c'est autres valeures sont �cras�es...
	...
	private void doStuffAuth(HttpServletRequest request,
		...
		//!!!! il faudrai mettre en place le process qui renseigne au moins les gens �tant �l�ve comme Isstudent et gens professeur comme etant IsTutor
